...
class RatingRepository extends ChangeNotifier {
  ...
  void deleteRating(String albumMbid) {
    database.db
        .execute('DELETE FROM ratings WHERE album_mbid = ?', [albumMbid]);
    notifyListeners();
  }

  List<Rating> allRatings() {
    final rows =
        database.db.select('SELECT * FROM ratings ORDER BY rated_at DESC');
    return rows
        .map((row) => Rating(
  ...
  Rating? ratingFor(String albumMbid) {
    final rows = database.db.select(
      'SELECT * FROM ratings WHERE album_mbid = ?',
      [albumMbid],
    );
  ...
  List<(Album, Rating)> ratedAlbumsMatching([FilterCriteria? criteria]) {
    final (whereSql, whereParams) =
        (criteria ?? const FilterCriteria()).toSqlWhere();
    final rows = database.db.select(
    ...
    return rows.map((row) {
      ...
      final album = Album(
        ...
        representativeReleaseMbid:
            row['representative_release_mbid'] as String?,
